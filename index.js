	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Tchal kkk @${num.split('@')[0]} Já estava na hora 😂👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				...
				only: {
					group: '📌 Este comando só pode ser usado por administradores de grupo! 📌',
			...
			switch(command) {
				...
				case 'info':
					...
					teks = `*𝙎𝙚𝙫𝙚𝙣 𝘽𝙤𝙩* : ${me.name}\n* wa.me/6285202626686* : @${me.jid.split('@')[0]}\n*Prefix* : ${prefix}\n*Contatos Bloqueados* : ${blocked.length}\n*Bot ativo desde* : ${kyun(uptime)}`
				...
				case 'meme':
					...
					break
				case 'Oii':
	              	if (args.length < 1) return reply ('Para mais informações digite .menu')
	            	break
				case 'Ola Seven':
	              	if (args.length < 1) return reply ('Olá, esse Bot foi editado pelo hike meu bem😏add ele ai wa.me/6285202626686')
	            	break
				case 'dono':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.imgur.com/6vjpZqf.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*NOME* HIKEE\n*WPP* wa.me/6285202626686\n*INSTA:* @hike_sutil\n🔥AQUI O SEVENN DOMINA🔥'})
					break
				/*case 'memeindo':
					memein = await kagApi.memeindo()
